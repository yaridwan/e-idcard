...
if(isset($post['simpan'])){
    ...
    $simpan = $this->db->insert("ttd", $post_data);
    if($simpan){
    ...
    redirect(base_url("master/ttd"));
}else if(isset($post['edit'])){
    $post_data = array(
        "nama" => $post['nama'],
        "jabatan" => $post['jabatan'],
    );
    $simpan = $this->db->update("ttd", $post_data, array("id_ttd" => $post['id_ttd']));
    if($simpan){
    ...
    $simpan = $this->db->delete("ttd", array("id_ttd" => $post['id_ttd']));
    if($simpan){
...
<body>
                                        ...
                                        echo"
                                            <tr>
                                                ...
                                                <td class='text-center'>
                                                    <div class='btn-group'>
                                                        <a href='#' class='btn btn-primary btedit' data-toggle='modal' data-target='#modalEdit' data-id='".$ddata['id_ttd']."' data-nama='".$ddata['nama']."' data-jabatan='".$ddata['jabatan']."'><i class='icon-pencil7'></i></a>

                                                        <a href='#' class='btn btn-danger bthapus' data-toggle='modal' data-target='#modalHapus' data-id='".$ddata['id_ttd']."' data-nama='".$ddata['nama']."'><i class='icon-trash'></i></a>
                                                    </div>
                                                </td>
                                            </tr>
                                        ";
    ...
    <div class="modal fade" id="modalEdit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <form method="post" class="modal-content">
                <input type="hidden" name="id_ttd" id="id_ttd_edit">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label>Nama :</label>
                        <input type="text" name="nama" id="nama_edit" class="form-control" placeholder="Nama" required>
                        <label>Jabatan :</label>
                        <input type="text" name="jabatan" id="jabatan_edit" class="form-control" placeholder="jabatan" required>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
    ...
    <div class="modal fade" id="modalHapus" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-sm" role="document">
            <form method="post" class="modal-content">
                <input type="hidden" name="id_ttd" id="id_ttd_hapus">
                <div class="modal-header">
